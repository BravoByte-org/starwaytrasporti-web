...
MCP authentication uses a separate static token configured in the user-level MCP client config (formerly `~/.cursor/mcp.json`; for pi, `~/.pi/agent/mcp.json` via pi-mcp-adapter) — never committed to repos.
...
| MCP Agent | Admin Access | MCP client (static token) | AI-assisted operations from an agent session (originally Cursor) |
...
- **MCP Agent** — dedicated Directus user for AI-assisted operations from an agent session (originally via Cursor)
